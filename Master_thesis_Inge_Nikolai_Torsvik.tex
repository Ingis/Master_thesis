...
The \ac{ADC} and the digital unit together constitute the so ALTRO chip. There are 16 PASA chips and 16 ALTRO chips on the \ac{FEC},
the PASA chip is connected to 16 readout pads each, which gives a total of 128 readout pads for one \ac{FEC}.
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{FEC.png}
  \caption{Layout of the ALICE experiment \cite{Carmen}}
  \label{FEC}
\end{figure}
...
This board is connected through a Ethernet ling to a computer on the outside of the ALICE detector.
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{RCU.png}
  \caption{Layout of the ALICE experiment \cite{Carmen}}
  \label{RCU}
\end{figure}
...
Since the infrastructure is basically the same, the placement of connectors and the size of the new \ac{RCU} should be the same as the old one. 
...
A Linux system is build on this Microcontroller Subsystem, which resulting in that we didn't need the \acf{DCS} board anymore.
The ProASIC was also not need for reconfiguration anymore, but was kept for radiation monitoring,
this part of the RCU2 is called RadMon, and consist of a ProASIC and SRAM cips.
Another issue encountered in the design was that a chip called TTCrx chip that was used as handling the clock and trigger signal on RCU1 was out of stuck and obsolete.
...
One of the limiting with the old setup was that the bus between \ac{RCU} and FECs was to slow.
...
The radiation environment at in \ac{LHC} is dominated by high energetic neutrons and protons.
