...
After a irradiation test at \ac{OCL} on a specific \ac{PCB}, the collected data consist of the exposed time, scintillator counts, current consumption and output data.
From the calibration before radiation test, we got the relation between scintillator counts and \ac{SEU} on the Neutron dosimeter. From earlier tests with the neutron dosimeter the cross section for a incoming proton to induce a \ac{SEU} is known. These data is used to calculate our way to the received dose for each of the different \ac{IC}. The next section will explain the calculation process.
...
\begin{equation}
1 \mega\electronvolt = 1000 \electronvolt = 1000 \cdot 1.602 \times 10^{-19}\joule = 1.602 \times 10^{-13}\joule
\label{MeV_to_joule}
\end{equation}
...
\ac{LET} into a material is equal the energy loss of the particle that crosses the material, which means that we can use the bethe bloch formula that is expressed in equation \ref{stopping_power}. The energy loss of a particle is highly energy dependent, but for short distances it can be assumed to be constant without getting to much error.
...
The \ac{DUT} was placed 130cm away from beam exit, we will therefore have some energy loss in the air between the beam-exit and \ac{DUT}. A program \cite{website:energyloss_calculator} was used to calculate the \emph{energy loss} for a 28 $\mega\electronvolt$ and a 25 $\mega\electronvolt$ proton beam in air, we get respectively $17.516 \frac{\mega\electronvolt\centi\meter^2}{\gram}$ and $19.201 \frac{\mega\electronvolt\centi\meter^2}{\gram}$. By multiplying with the density of air which is $\rho_{air}=0.001275 \frac{\gram}{\centi\meter^3}$ we get $22.333 \frac{\kilo\electronvolt}{\centi\meter}$ and $24.481 \frac{\kilo\electronvolt}{\centi\meter}$. This means that the energy of the protons when it hits the \ac{DUT} are:
...
\begin{equation}
E_{proton2} = 25 - (130 \centi\meter \times 24.481 \frac{\kilo\electronvolt}{\centi\meter}) \approx 22 \mega\electronvolt
\label{Proton_energy2&3}
\end{equation}



If we look at a single proton entering a \ac{IC} (which consist of mostly silicon), the energy deposited by the single proton can be expressed as,
...
\begin{equation}
\rho_{Si} = \frac{\gram}{\centi\meter^3}
\label{silicon_density}
\end{equation}

To get the total energy deposited in the \ac{IC}, we have to multiply the energy of a single proton multiplied with the flux and the area of the \ac{IC}, then we have:

\begin{equation}
\Delta E_{total} = \Delta E_{proton} \cdot \Phi \cdot A = \frac{dE}{dx} \cdot \Delta x \cdot \rho_{Si} \cdot \Phi \cdot A
