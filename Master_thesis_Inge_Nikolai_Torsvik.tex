...
At \acf{CERN} in Switzerland there are being conducted experiment on fundamental structure of the universe. This is done by accelerating particles up to a energy of 4 $\tera \electronvolt$ per proton, and then crash with a other particle with same energy. This experiment is done by connecting several accelerators with higher and higher energies together to accelerate particles. The largest one is called \acf{LHC}, and is the largest particle accelerator ever built, installed in a 27 $\kilo \meter$ long tunnel.
To detect what is happening to the particles when crashed, there has been built several detectors that is placed in the tunnel. One of these is called \acf{ALICE}.
...
My work through this thesis has been planing how to test different circuits, designing test setup and executing the tests.
For most of the circuits, I designed a \ac{PCB} with only the necessary to make the circuit work and to be able to test it.
...
For the more advanced \ac{IC} like the \ac{SF2}, I used a starter-kit when designing the test, and wrote test code in VHDL and C.
I also made a \acf{PCB} for measuring current and control a power regulator on the \ac{SF2} starter-kit.
The circuits I have tested consist of: SoC FPGA, power regulators, bus transceivers, limiting amplifier, multiplexer/demultiplexer and buffer, comparator and Current Shunt Monitor.
Every test board I made was radiation tested in \ac{OCL} at the University of Oslo, the \ac{SF2} chip was also radiation tested in \ac{TSL} in Uppsala in Sweden.
...
up to their maximum energy of 4 TeV and speed up to the speed of light. When particles has reached this energy level accelerated particles from opposite direction are made to collide inside 4 different experiment areas, where one of these is called \acf{ALICE}. See figure \ref{ALICE_layout}.
...
Under a such collision a temperature 100 000 times higher than the temperature of the sun is generated.
...
\footnotetext[1]{All ordinary matter in today's universe is made up of atoms.
...
A current signal given by one of the pads, is sent into the \acf{FEC} which consist of three basic functional units, see block diagram in figure \ref{FEC_blockdiagram}.
The first unit is a charge sensitive amplifier/shaper called PASA, the second unit is a 10-bit 25MHz low-power \ac{ADC}.
The last unit is a digital circuit that perform the baseline subtraction, tail cancellation, zero-suppression \footnotemark[2], formatting and buffering.
...
\footnotetext[2]{Zero Compressions means that signal bellow a given threshold will be filtered away.}
\FloatBarrier
\subsubsection{Readout Control Unit RCU}
One \acf{RCU} is connected to one row of \acp{FEC} (up to 25 pieces), through a backplane\footnotemark[3], see figure \ref{TPC_Sections}.
...
\footnotetext[3]{A backplane is a PCB board, that connects all of the Front End Cards to the Readout Control Unit.
...
\paragraph{The new Readout Control Unit, the RCU2} $\newline$
The main motivation for the \ac{RCU2} was to develop a solution that gives the needed performance improvement,
...
Since the infrastructure is basically the same, the size and placement of connector on the new \ac{RCU} is the same as for the old one. 

The main difference between the \ac{RCU2} and \ac{RCU}1 is that the main \ac{FPGA}, the Xilinx Virtex-II Pro,
has been replaced by a Microsemi \acf{SF2} \acf{SoC} \ac{FPGA} M2S050-FG896. This is a flash-based FPGA which has
\ac{SEU} immune configuration memory, as well as several other radiation tolerance measures implemented.
...
\section{Preparations for Testing}
...
\subsection{The \acfp{IC} Which Where Tested}
...
For each of the different \ac{IC}, except for SF2, a \acf{PCB} was made with only the necessary electronics to make it work and to be able to test the functionality of the \ac{IC}.
Connectors (1-2) was placed on the \acp{PCB} and connected to \acf{DAQ} boards from National Instruments. These gave us the possibility to control the inputs of each \ac{IC} and monitor the outputs.
...
There may be some difference between two boards made for the same chip, that is because the first board made may have some modification on the \ac{PCB}, to make it work. When the second board was made, error or mis functions was fixed at \ac{PCB} level. So that modification wasn't needed.
...
In figure \ref{TPS51200_sch} you can see a schematic layout of the \ac{PCB} board for the TPS51200.
...
This is a adjustable regulator. That means that by changing R1 and R2(see figure \ref{MIC_sch}) you can adjust the output voltage, see equation \ref{Vout}. For the two test boards we had a input voltage of 3.3 V.
...
An application example could be to convert a 16-bit digital signal that with high value of 1.5 V to a 16-bit signal with high value of 3.3 V.
The input and output high value can be set to anything between 1.4 and 3.6 V, the low value is set to 0 V.
...
\subsubsection{All the test boards}
In figure \ref{PCB_board1}, \ref{PCB_board2} and \ref{PCB_board3} you can see how the PCB boards looked like. 
...
\subsection{Software}
For each of the different test board that was made a simple labVIEW program was made, special designed to supply and monitor a specific \ac{IC} under radiation.
...
\subsection{SmartFusion2}
\acf{SF2} \ac{SoC} \ac{FPGA} integrate a flash-based \ac{FPGA}\footnote{Flash-based FPGA means that configuration registers is saved in flash memory cells.
Compared to SRAM-based FPGA where configuration is saved in SRAM cells, flash-based FPGA is much more tolerant against radiation.},
an ARM Cortex-M3 processor built in a \acf{MSS} and high-performance communication interfaces on a single chip.
...
The SRAM blocks on the SF2 are divided up into 72 micro SRAM blocks with a size of 64 x 18 bits, and 69 Large SRAM block with a size of 1024 x 18 bits.
...
The last state is a state where all of the addresses are written, read and compared through an endless loop. The written data to an address is always set to be opposite of the previous value written to that address. The state machine will stay in this state until a reset is pushed.
...
\subsubsection{Test of the Logical element} $\newline$
...
When a logical element is exposed to radiation, a \ac{SET} can occur all over the chip.
If this happens in the logical element or in the interconnections between logical element, the transient current will normally just be there for a small amount of time, and then it will flat out, causing no error or bit flip in the chip. But if this transient current hits a sensitive node as the input to a flip-flop, or a node that may lead to a change of value at the input, and the input is then clocked out to the output, we would have a unwanted bit change or a so called \ac{SEU}.
...
For our tests we made 4 shift-register chains, with 4 inverters in between each flip-flop and where each of the shift-register had a 4-bit \ac{WSR} output. The length of the shift-registers were set to 2000.
...
\subsubsection{Single Event Latchup and PLL} $\newline$
...
The PLLs on the SF2 has a lock signal which checks if the output of the PLL is in sync with the input. If it is in sync, the lock signal is high, if not, its low. 
...
Two types of serial communication interface are available, that is USART and SPI, which is selectable in software.
Since the microcontroller only has a 16-bit architecture, only 16-bit of the \ac{ADC} is accessible at a time. The microcontroller has 3 differential \ac{ADC} inputs, which means voltage over three resistors can be measured.


To measure current for the SF2 starter-kit a small shunt resistor of 0.1 Ohm was placed in series with the supply signal on the starter-kit, one for the 1.2 V supply voltage, and one for the 3.3 V supply voltage. The voltage over this resistor was measured with the \ac{ADC}, and the \ac{ADC} data was sent over USART port to the serial to USB converter, and further to a computer. On the computer a labVIEW program was set up to receive data through the serial port, and when data is received, the \ac{ADC} value and the known resistor value is used to calculate the current. The current value is constantly saved and exposed for a viewer.
...
If the current on one of the supply voltages goes over a certain threshold, the bit on the microcontroller is set to go low, resulting in turning the 3.3 V regulator on the SF2-starter kit off. The 3.3 V is used to power all other regulators, so by turning this regulator off, the whole starter-kit is power less. 

\subsection{Equipment used for testing}
...
\subsubsection{Neutron dosimeter}$\newline$
The Neutron dosimeter boards are made by Arild Velure, as part of his master thesis \cite{arild_master}.
It is a SRAM-based radiation monitor. The cross section(the probability that a incoming particle will induce an SEU) is known, and is found to be to be $\unit{1.14\e{-6}}{\centi\square\meter}$ for protons. by measuring \ac{SEU} under radiation, we are able to calculate the fluence\footnote{Fluence is the total number of particles that intersect
a unit area in a specific time interval of interest, and has units of $particle/m^{-2}$ (number of particles per meter squared)} of the beam, since the cross section is given.

The Neutron dosimeter is a \ac{PCB} with a flash-based ProASIC FPGA, four 16 Mbit Cypress SRAM chips (CY62167DV30LL-55ZXI), connections and supporting electronics.
The \ac{FPGA} is designed with RS485 two-way communication which makes it possible to edit firmware, send commands and get data out.
Communication to a computer is done through a Opal Kelly XEM3001 board, which converts from RS485 to USB.
A labVIEW program is following the board. Here we are able to reset data, do some basic settings, and monitor data.
The program also saves data onto the disk.

The approach for detecting an Single Event Upset (SEU) in the \ac{SRAM} is rather straight
...
to the known value, and one or more of the bits are not equal a \ac{SEU} has occurred, and a counter will be incremented for each bit error.
The correct value is then written back to the address and the system moves on to the next address.
  
A checkerboard pattern of alternating ones and zeros, is used when writing
...
Computer  & A VNC server was set up on a computer inside the experimental hall, this made us able to control the experiment from the control room. The computer was running all the necessary software to control the experiment. \\ \hline
