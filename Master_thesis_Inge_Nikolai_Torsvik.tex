...
When on of the pads get hit by a ionized particle, it sends ut a current signal with a rise time of less than 1 ns and a little slower falltime due to the motion of the positive ions.
The amplitude is slightly different for the different pad sizes, but has a typical value of around 7 $\micro\ampere$.
A current signal detected by one of the pads, is sent into the \acf{FEC}, which consist of three basic functional units, see block diagram in figure \ref{FEC_blockdiagram}.
...
The \ac{ADC} and the digital unit constitute the ALTRO chip. There are 16 PASA chips and 16 ALTRO chips on the \ac{FEC},
the PASA chip is connected to 16 readout pads each, which gives a total of 128 readout pads for one \ac{RCU}.
...
One row of \acp{FEC} (18 to 25 pieces) is controlled by a \acf{RCU}.
The \acf{RCU} consist of a the motherboard, the DCS board and the SIU board.
Most of the \ac{RCU} functions is controlled by the main FPGA, which is a Xilinx Virtex-II Pro FPGA.
this \ac{FPGA} is in charge of data readout from the \ac{TPC} 
