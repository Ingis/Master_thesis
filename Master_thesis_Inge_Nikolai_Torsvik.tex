...
For each of the different \ac{IC}, except SF2, a simple \acf{PCB} was made as a platform to be able to send in input data and take out the outputs.
...
A small resistor was placed in series with the power input. By measuring volage drop over this resistor we could calculate the current using \emph{Ohms law} (I = U/R).

We wanted to have two \ac{PCB} for each of the \ac{IC} that were going to be tested. That was to have more test data on each \ac{IC}, and as a precaution if one of the boards wouldn't work.
...
The analog outputs has a limit of 5 $\milli\ampere$, but some of the \acp{IC} that were tested required more than that. For these cases the 5 V output, that can deliver current up to 200 $\milli\ampere$, and a voltage regulator to 3.3 V was used, see section \ref{MIC_explanation}.
...
There are a few differences between the two \acp{IC}. MAX3748 comes in a smaller package and uses less power and works in higher rates, but it doesn't have a clock and data recovery function as as ADN2814 has. For MAX3748, we will need to do the clock and data recovery function in the main SF2 FPGA, using some of the FPGA logic. The clock and data recovery function in ADN2814, is actually clock recovery and data re-timing, that means that the output data is actually the same as input data, except that it is in LVDS format. This means that we have to use FPGA logic to recover data, but since we have the clock signal, it is easier to decode and extract the data signal.

To be able to make a good test for these two, we used a SF2 starter-kit. The SF2 FPGA on the starter-kit were used to encode a clock and data signal into a differential Manchester signal that where transmitted to the two circuits. The manchester signal was used as a input for the two circuits, and a LVDS clock and Machester signal was sent back from the ADN chip, and a CML Manchester signal were sent back from the MAX-chip to the starter-kit. The Manchester signal coming back had to be decoded back to clock and data, this was done in the SF2 FPGA. To be able to check if the returning data is equal the original, the original data signal had to be delayed, since sending data out, through a IC and back contribute to some delay. The original data was delayed by sending it through a few D-latches, until the two data signals had the same phase. Then we could start comparing, this were done through a XOR-function, triggered by a 80 Mhz clock. Every time the XOR function goes high, that is when the compared data is 
...
The interesting thing when determining if a \ac{IC} tolerant towards radiation is the dose. In run2, it has been calculated that the total dose in the TPC is 1-2 kRad. This is the absolute limit a \ac{IC} have to survive, and a safe margin on at least 3 times should be used. We also have to take account that when radiating at OCL, we have been using proton beam on less than 30 MeV, in the TPC detector in ALICE, we can receive protons and neutrons in the energy level of higher than 1 GeV, which may cause other effects that we have seen in our test at OCL. It is therefore important to have a good margin on the dose.
...
The calculated values for cross section per bit with uncertainty can be found in the table \ref{Cross_section_calc1}. The total number of bits in the SRAM are 622592 and in the micro SRAM there are 65536.
...
On run two, we ran 4 shift register chains with shift register length of 2000, with 4 inverters in between each register and 4 windowed shift register. The frequency used was 40 Mhz. In table \ref{SR_table}, you find the results from run2.
...
\begin{equation}
 CS = \frac{\frac{PLL loss}{2}}{Fluence} = \frac{23}{1.36\cdot10^{11}} = 8.46 *10^{-11}
\end{equation}
...
The SRAM test gave us around the expected numbers of SEUs. We should have tried to run the test without going through the MSS. If we had tried to send data directly out from the FPGA, we would get rid of all the problems with the MSS.
...
Instead of using the current measurement board as described in section \ref{current_measurement_board}, we used a small PCB with severals INA226, which is a ADC with $I_2C$ communication protocol. The SF2 has $I^2C$ communication protocol, which means that we could use the monitoring board to control and receive data from the ADC-chip. By using this method we also got the voltage on each of the different power source that was measured. The monitoring board were also connected to the enable pin of the 3.3 V voltage regulator on the test board, making it possible to turn on and off the starter-kit under test at will. Every tenth of a second the status of data was written to a console and to a file, which made it possible to monitor as the tests where running, and look at the data afterwards.
...
The first PLL loss occurred after a dose of 0,043 kRad for the first test and after 1.332 kRad on the second, which is a fairly low dose. This means that PLLs could be a problem for the SF2 design. If possible, we should try and not use PLLs, or have some kind of redundancy, when using these.
...
\begin{equation}
 CS = \frac{\frac{PLL loss}{3}}{Fluence} = \frac{176}{2.39\cdot10^{11}} = 2.45 *10^{-10}
\end{equation}

\begin{equation}
 CS = \frac{PLL loss}{Fluence} = \frac{205}{2.93\cdot10^{11}} = 2.33 *10^{-10}
\end{equation}
...
\subsubsection{Logic element}
Through the test on the two starter-kit we run at 40, 80 and 160 Mhz. Which means that we should see some difference in upset detected by the shift register design.
In table \ref{Shift_register_data} you can see a overview of all SEUs on the shift register design with a given frequency over a given time and dose.

\begin{table}
 \centering
 \begin{tabular}
  {|l|l|l|l|l|}  \hline 
 Frequency & inverters in between & Received dose* & Shift register errors & SR errors/dose  \\\hline\hline
 40 Mhz & 4 & 8.52 & 51 & 5.99  \\\hline
 80 Mhz & 4 & 5.88 & 26 & 4.42 \\\hline
 40 Mhz & 0 & 4.18 & 19 & 4.54 \\\hline
 80 Mhz & 0 & 4.31 & 30 & 6.96 \\\hline
 160 Mhz & 0 & 9.78 & 32 & 3.27 \\\hline
 \end{tabular}
\caption{}
...
When exposing the optical receiver, which receives the timing, trigger and clock signal, we saw PLL lock lossese all the time. This means that the current setup which are now being used is way to bad, and something needs to be done to fix this.

Another discovery done through these test, is that by reducing the voltage on the 1.2 V power source, we saw a reduction in latchups, without any reduced functionality.

The serdes test gave us good result, which means that the current setup is good enough.
