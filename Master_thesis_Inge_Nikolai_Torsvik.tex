...
\label{how_to_test}
The radiation in the \ac{LHC} is dominated by high energetic neutrons and protons, mostly neutrons with a estimated fluence of $(0,6-1,1)\times 10^{11}$ $neutrons/cm^2$. Therefore it would be preferable to test our electronics with a neutron beam, but since there are few labs who can produce a neutron beam compared to proton beam most of the electronics is only tested at OCL with a proton beam.
...
Everything in this chapter is based on these references \cite{rad_phys}, \cite{Weste_harris}, \cite{knoll} and \cite{balashov} if not otherwise stated.
...
If not taking seriously into account, the radiation effects may damage or even destroy the electronics.
Therefore there is if highest importance to know about how irradiation can effect the electronics.
