...
The board can be connected to a computer through a Opal Kelly XEM3001 board, which converts the RS485 signal from the \ac{FPGA} to USB.
...
The first time at \ac{OCL} we were given an beam of 28 MeV, but the second and third time the lab personnel manage only to produce a beam of 25MeV.
...
TSL is a facility located in the University of Uppsala in Sweden.
...
\section{Results and calculations from beam test at OCL and TSL}
In this chapter the result after radiation test at OCL and TSL is presented.
The result is presented after type, so if two \ac{PCB} that the two boards of the same type is presented together. 

\subsection{Results from OCL}
...
The laser was first placed in a position, so that the laser beam was though to hit center. 
Then We placed a film in front of the beam exit and \ac{DUT}. By seeing how the film turned black, we got a felling of where the center was.
...
Now we had a feeling of where center is compared to where the laser was pointing. The Neutron dosimeter was placed with one of the SRAM chips at where the laser was pointing. This position was called position zero (x,y) = (0,0). From the result from the film, we knew approximately how much we had to move to be in center.
