...
It is manly used for proton therapy of cancer patient by Uppsala University Hospital, but is also used for medical research and radiation testing for electronics systems.
The heart of the installation is a Gustav Werner cyclotron that delivers beams of charged particles, from proton to highly charged xenon ions of energies up to 192 MeV.
The experiment is executed in the so called \emph{Blue Hall}, where there is two extraction points, one used for protons, and one used for neutrons and heavier ions, see figure \ref{Blue_hall}.
The cyclotron and the control is done from the control room.
...
Two ways to calculate dose will be discussed in this section, that is manually using energy loss and \ac{LET}, see section \ref{radiation_phys}, and using simulation data from a program named FLUKA. Which of these gives us the most reliable result will be discussed at the end of this chapter.
...
\begin{equation}
1 Rad = \frac{Gy}{100} = 0.01 \frac{\joule}{\kilogram}
\label{Gy_to_Rad}
\end{equation}
...
By looking at the total \ac{LET} to a \ac{IC}, we are able to calculate our way to the dose.
...
Two energies was used when exposing circuits at \ac{OCL}, The first time we had a proton beam with a energy of 28 MeV, the two other times we had a beam of 25 MeV.
The \ac{DUT} was placed 130cm away from beam exit, we will therefore have some energy loss in the air between beam-exit and \ac{DUT}. A energy loss calculator program \cite{website:energyloss_calculator} was used to calculate the \emph{energy loss} for a 28 $\mega\electronvolt$ and a 25 $\mega\electronvolt$ proton beam in air, we get respectively $17.516 \frac{\mega\electronvolt\centi\meter^2}{\gram}$ and $19.201 \frac{\mega\electronvolt\centi\meter^2}{\gram}$. By multiplying with the density of air which is $\rho_{air}=0.001275 \frac{\gram}{\centi\meter^3}$, we get $22.3 \frac{\kilo\electronvolt}{\centi\meter}$ and $24.5 \frac{\kilo\electronvolt}{\centi\meter}$. This means that the energy of the protons when they crash into \ac{DUT} are:
...
By using the same energy loss calculator we get that the energy loss in silicon (which a \ac{IC} mostly consist of), we get:
...
To get the total energy deposited in the \ac{IC}, we have to multiply the energy of a single proton multiplied with the fluence, which is the total flux over a given time,  and the area of the \ac{IC}, then we have:

% Flux = \Phi
\begin{equation}
\Delta E_{total} = \Delta E_{proton} \cdot \Phi \cdot A = -\frac{dE}{dx} \cdot \Delta x \cdot \rho_{Si} \cdot fluence \cdot A
\label{total_energy_loss}
\end{equation}

Now we know the total energy loss for a proton beam. And can start calculating the dose. Dose is energy per mass, as said in section \ref{difinitions}. That means that the absorbed Dose in silicon can be expressed as,
...
To get the dose in Rad instead of $\frac{MeV}{g}$, we have to multiply with the converting factor given in equation \ref{MeV_to_joule}, which gives us the dose in Gray. To get the dose in Rad we multiply by 100. This gives us:

\begin{equation}
Dose[Rad(Si)] = 1.602 \cdot 10^{-13} \cdot -\frac{dE}{dx} \cdot fluence \cdot 100
\label{Dose_rad}
\end{equation}

\subsection{Calculating dose using FLUKA simulations}
The program Fluka can be used to simulate particles with a user given energy in whatever material you want. We simulated a proton beam with energy of 28 MeV and 25 MeV in air, and set our \ac{DUT}-position to 130 $\centi\meter$ away from beam exit. The results can be seen in table \ref{FLUKA_simulations}.
...
The procedure to calculate the dose are first to find the proton fluence, by first converting from scintillator counts to \ac{SEU} by multiplying with the converting factor found as explained in the beam setup, see chapter \ref{beam_setup} and appendix \ref{Beam_profile} for the results. Since the cross section is known for the SRAM detector, we are able to calculate our way to the fluence and flux, see equation \ref{FluenceDUT} and \ref{flux}.
...
Now we can take use of the Fluka simulation results. The simulation results tells us how many protons will hit \ac{DUT} when one proton will be sent out from the beam exit, and what dose this will give the \ac{DUT}. By using the calculated Fluence on DUT value, found in equation \ref{FluenceDUT}, and dividing on \emph{primary particles at \ac{DUT}} from the simulation results, we find the Fluence at beam exit, see equation \ref{FluenceBE}. Then we can use \emph{dose per primary particle at DUT} from the result and multiply with the Fluence at beam exit point, which gives us the dose in gray for the \ac{DUT}, see equation \ref{DoseDUT}. By dividing on 100 we get the dose in Rad.
...
To compare LET calculations with FLuka simulation we need an example to look at.
If we look at the test result from radiation of MAX3748:
...
The first thing to do is calculate energy when entering the integrated circuit 130 cm away from beam exit. This is found in equation \ref{proton1} to be 22 Mev.
LET or energy loss for a 22 MeV proton particle in silicon, is calculated in equation \ref{energy_loss_2}, which gives us 17.1 MeV.

By multiplying the scintillator count value with the scintillator conversion value, we get SEU on SRAM to be = 485041. By dividing SEU on cross section we get the fluence at DUT.

\begin{equation}
FluenceDUT = \frac{485041}{1.14E-6} = 4.25E11
\label{FluenceDUT_MAX3748}
\end{equation}

By using equation \ref{Dose_rad} we get the total dose in Rad to be,

\begin{equation}
Dose[Rad(Si)] = \frac{1.602 \times 10^{-13} \cdot -\frac{dE}{dx} \cdot fluence}{100}
\label{Dose_rad}
\end{equation}
