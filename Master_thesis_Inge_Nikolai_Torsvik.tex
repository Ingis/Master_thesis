...
\centering
  ...
  \begin{subfigure}{.49\textwidth}
  \centering
  \includegraphics[width=0.65\textwidth]{figures/backside_PCB}
  \caption[PCB Boards 1]{Picture of PCB boards, from left we have, TPS51200, MIC69302WU and SN74AVCB164245}
  \end{subfigure}
 \caption{PCB-boards}
 \label{PCB_board3}
...
\subsubsection{SRAM}
\autoref{SRAM_starter-kit1} and \ref{SRAM_starter-kit2} shows the results from the SRAM test after radiation of the two starter-kits. We detects regularly drops down to 0 in the counter values. This is due to loss of communication with the microcontroller subsystem, and thereof the SF2, probably caused by voltage drop or latchup as discussed in the previous subsection.
...
\section{Summary}
The work presented in this thesis was carried out in order to investigate the radiation tolerance for components used in the design of RCU2. The main focus has been on the RCU2's main FPGA, the SmartFusion2 SoC FPGA, which have been tested for \acl{SEE} like \acl{SEU}, \acl{SET} and \acl{SEL} on the internal SRAM cells, logic elements and PLLs. The other components that have been tested are TPS51200, MIC69302WU, SN74AVCB164245, SN74AVC2T245, QS3VH257, SY89831, ADN2814, MAX3748, INA210 and TLV3011, but then the focus has been on accumulative effects.
...
In the beginning of this work, time was spent on learning about radiation and radiation induced effects on semiconductor devices, so that sufficient knowledge was acquired to make a good test setup and procedure. A lot of time was also spent on getting to know the different components, so that we would know how to be able to test them in a good manner.

For all of the tested components except SmartFusion2, a simple PCB was constructed as a platform to be able to send input data and measure output data from a component, this was done with USB-\acl{DAQ} devices from National instruments. For the test of SmartFusion2 SoC FPGA a SmartFusion2 starter-kits was used as a platform.

% This knowledge was also used to determine what the detected effects we saw during the irradiation.

There have been a preparation period where simple PCB has been made to work as a test


There have been conducted radiation test four times during this work, three times at Oslo Cyclotron Laboratory with energies of 28 and 25 MeV and 1 time at The Svedberg Laboratory with an energy of 170 MeV. 
