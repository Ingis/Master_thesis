...
To be able to make a good test for these two, we needed to use a SF2 starter-kit. The SF2 starter-kit was used to make a Manchester-coded signal, which was sent from the starter-kit with wires to a \acp{PCB} with the ADN2814 or MAX3748. The output data from the component are sent back, where the Manchester-coded signal will be decoded and compared with the original clock and data (Clock and data recovery function on ADN2814 was not used). To compare the input and output clock and data, an internal clock from the FPGA, which is 90$\degree$ phase shifted from the incoming and outgoing clock signal to the \ac{PCB}, was used to trigger a XOR function which will go high if the two compared signal are not equal, incrementing a counter.
...
Two boards was tested. The first board was tested 15.11.13 and the second board was tested 28.11.13. During the test of the first board we increased the intensity of the beam quite drastically at the end of the irradiation, this can be seen from the flux graph, in figure \ref{flux_all1}.
