...
Testing \acfp{IC} are a process that has been done many times before, in conjunction with design of electronics that are going to be installed in the \ac{LHC} at CERN. This chapter will go trough all the components that are going to be tested, and explain how to test.
...
On the RCU2 this is going to be used to regulate a the 3.3 V power signal down to 1.2 V power signal. The 1.2 V power signal is manly used to power the main FPGA, the SF2, on the RCU2.
...
A third \ac{PCB} was made with this \ac{IC}. This was used to to supply 3.3 V to the components which requires more than 5 mA, which is the maximum the analog outputs the USB-\ac{DAQ} can deliver.
...
To get the counter values and cycle counter data from registers in the FPGA to a computer, the \acf{MSS} was used. The \ac{MSS} uses APB bus to collect the counter values, the microcontroller are sending the counter values through UART, which is connected to a serial to USB converter and further to a USB-plug. By connectinga computer to the USB-plug, we get serial communication to the microcontroller. A C-code was written to the microcontroller, which in short sets up the UART communication and constantly sends \ac{SEU} counter and cycle counter values through UART and out to a computer, and checks for received data. The received data is used to determine the frequency, of the shift-register design, as described in section \ref{logic_Element}.
...
And the third period, the focus was mainly the \ac{SF2} starer-kit, but $INA210_1, INA210_2, TLV3011_1 and TLV3011_2$ was also tested.
...
This computer was used as a connection point to our test boards. We sat in a room called \emph{countingroom}, where we could turn the beam on and off as we desired. There were also a machine in the countingroom, which was connected to detectors in the blue hall telling us the fluence (number of protons per $\centi\meter²$ in total) and exposed time. The fluence can be used to calculate the flux and dose. The energy of the proton beam we got was 170 MeV.
...
\begin{itemize}
  ...
  \item Uncertainties from errors ($\sigma{err}$) occurred during the run, is estimated to be abut 10 \%.
...
In table \ref{PLL_error1} and \ref{PLL_error2} you can see the result from the PLL test. The The PLL that was monitored was driven by another PLL. Thus, assuming that if the first PLL looses its lock signal and thus clock, the second PLL will also loose its lock signal. We got a total of 23 PLL lock loss, giving us 11.5 loss per PLL over a dose of 45 kRad. A noticeable things is that the first lock loss already occurred after a dose of 0,7 kRad. We also have a period between 5.0 and 18.7 where we didn't detected any PLL lock loss at all, which is rather strange.
...
In this section the results form the irradiation test at TSL will be presented. The focus has been on the SF2 test, but other test will briefly be presented and discussed.
...
Some improvements has been done to the the test since the radiation at OCL. We have added two more PLL lock signals to monitor, and we have found a more effective way to measure current.
Instead of using the current measurement board as described in section \ref{current_measurement_board}, we used a small PCB with severals INA226, which is a ADC with $I^2C$ communication protocol. The SF2 also has $I^2C$ communication protocol, which means that we could use the monitoring board to control and receive data from the ADC-chip. By using this method we also got the voltage on each of the different power source that was measured. The monitoring board were also connected to the enable pin of the 3.3 V voltage regulator on the test board, making it possible to turn on and off the starter-kit under test at will. Every tenth of a second the status of data was written to a console and to a file, which made it possible to monitor as the tests where running, and look at the data afterwards.
...
A issues that we uncounted here, and not so much in OCL, is latchup. We got regularly latchup during our run, which means that a shut down is required to set the board in normal operation again. A solution for this problem has already been set in emotion. By using \emph{INA210} to amplify a voltage over a shunt resistor on each power signal, and use \emph{TLV3011} to compare against a voltage limit. If the voltage limit is exceeded all regulators are turned off, setting the board off. The problem here is that when we actually have allot of radiation, and the probability of a latchup is high, that is when we are most interested in data. A solution here could maybe tolerate one or two latchup, or 

The third issue is the clock and data recovery setup. We experienced some errors when exposing the SF2, but 
