...
The amplitude, is slightly different for the different pad sizes, but has a typical value of around 7 ?A.
A current signal detected by one of the pads, is sent into the \acf{FEC}, which is comprised of three basic functional units, see block diagram in figure \ref{FEC_blockdiagram}.
...
and zero-suppression unit, and a multiple-event buffer. The \ac{ADC} and the digital unit constitute the ALTRO chip.
Each \acf{FEC} has 128 channels which are connected to the readout pads. An example picture of a \ac{FEC} can be seen in figure \ref{FEC}
...
The \acf{RCU} consist of a the motherboard, the \ac{DCS} board and the ac{SIU} board.
