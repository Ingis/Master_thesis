...
On the Microcontroller Subsystem we are able to build a Linux system, which replaces the functionality of the \ac{DCS}-card.
...
It is therefore of highest importance to know about these effects, how they affect the electronics, how much damage they can cause and how we can protect and prevent the radiation effects to do damage.
...
In the next sections we will look more closely into how a charged particle and neutral particle interact with matters.
...
If we have a particle with a given energy passing into a material, where $dE$ is the mean energy that the particle loses by traveling through a path segment, $dx$ of the material.
...
We normally divide radiation effects into two groups; $\emph{Singel Events effects}$ and $\emph{Accumulative Effects}$.
...
In the following sections we will look into three \ac{SEE}, which are \acf{SEL}, \acf{SET} and \acf{SEU}.
...
but other devices may not even have any effect after a severe dose of radiation. When a chip stops working, we say that it has reached its tolerance level.
...
By comparing a proton beam with a neutron beam of 21 MeV we see that there are 10-25\% less \ac{SEU} cross section for a proton beam compared to a neutron beam.
If we increase the energy to 88 MeV then we get close to none deviations. 
