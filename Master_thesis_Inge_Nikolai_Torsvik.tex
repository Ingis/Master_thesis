...
The work described in this thesis have been carried out within the Microelectronics reasearch Group at the Department of Physics, University of Bergen between August 2013 and June 2014.
...
Ketil R\o ed deserves special attention for his advices and endless discussion throughout my thesis despite not being my supervisor. Your discussions have given me better understand of radiation and calculation of dose, and helped me write my thesis in a good manner.
...
I would also like to thank my adorable wife, Solveig Str\o msnes for being patient, for support and encouragement and to be there for me when I have been discouraged and out of energy.
\newline\newline
Bergen, June 2014 \newline\newline
...
An overview of the tests can be found at the end of this section, with the total exposed time, the total accumulated dose and error status. For information regarding flux for the different tests, see Appendix \ref{radiation_results} \autoref{flux_all1}, \ref{flux_all2} and \ref{flux_all3}
...
A challenge when we exposed the test boards, is that the proton beam didn't seem to be stable, this can be seen from the flux graphs in \autoref{flux_all1}, \ref{flux_all2} and \ref{flux_all3}.
...
In Appendix \ref{radiation_results} some graphs of current vs time, it can be seen that after the beam was turned off at the end of the radiation, the current starts decreasing. This is a annealing effect. This could mean that during stops in the ALICE experiment components may recover and are therefore be able to tolerate a even higher dose then what is found through this work.
...
To know how often we can expect to encounter the different Single event effects errors when the RCU2s will be mounted in the TPC, we will calculate MTBF, as we did after the result in OCL. See \autoref{MTBF} for how MTBF is calculated. We also have to scale up for 216 RCU2s, 56340 registers, 622592 large SRAM bits, 65536 micro SRAM bits and 1 PLL. The results can be found in \autoref{cross_section_results2}.
...
Disregarding the result from the limiting amplifier, ADN2814, we didn't detect any output error during any of the other tests before a radiation dose of $85\ kRad$, which is more than 40 times higher than what is expected in the TPC environment. For all of the tests, we didn't detect any noteworthy current increase before a dose of $30-40\ kRad$, which is 20 times higher that what it will receive in the TPC. Therefore all the tested components have been cleared to be used on the RCU2.
