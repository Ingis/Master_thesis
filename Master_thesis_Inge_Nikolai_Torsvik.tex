...
This is the last accelerator in a chain of up to 7 (depending on which particle to accelerate), see picture \ref{CERN_accelerators}, where the particles gradually accelerate to higher and higher energies, up to their maximum energy of 4 TeV and speed up to the speed of light. When particles has reached this energy level accelerated particles from opposite direction are made to collide inside 4 different experiment areas, where one of these is called \acf{ALICE}. See figure \ref{ALICE_layout}.
...
\section{The \acfp{IC} Which Where Tested}
...
The last state is a state where all of the addresses are written, read and compared through a endless loop. The written data to a address is always set to be opposite of the previous value written to that address. The state machine will stay in this state until a reset is pushed.
...
Here the data received are sent through one of the \ac{UART} serial port which is connected to one of the USB-port on the starter-kit. A labVIEW program are made to read out the \ac{SEU} and cycles values, and exhibited for the viewer, data is also saved to disk.
...
The Clock Conditioning Circuit (CCC) used to produce the clock is configurable through a register, which can be accessed through the \ac{MSS}, so that the frequency can be changed without reprogramming the \ac{FPGA}.
...
All test was put in one project. 


\subsubsection{MSS and labVIEW}
...
This chapter will give a brief introduction to the two facilities, and explain how to prepare for beam test, how the setup looked like and how monitoring is done through a test.

\subsubsection{Purpose of tests}
The purpose of testing the RCU2 electronics for radiation are to see if it are able to survive in a radiated area as we will find in the \ac{LHC}, see section \ref{how_to_test}.
