...
In this subsection the results from radiation test on the SF2 M2S050ES chip will be presented. The different parts which where tested will be presented for them self, and we will have a discussion on the total result at the end. Only one starter kit was tested, but the beam was turned off some times during the test. After a dose of approximately 37 kRad we got a very high increase in current, probably caused by a latchup. We did a re-power of the board, and the current went back to a normal level, so we continued the test until a dose of 45 kRad when the current increased one more time, but this time it was permanently, even a week after radiation, the current was still high.

We had some problems with the \acf{MSS}, which stopped working in periods. What we had to is to restart the MSS, but then we had a short period of lost data, this can be seen as fall in the \ac{SEU} and cycle counter and small fall in current \ref{SRAM_test1}. This was mostly discovered fast, and the MSS was restarted.

\subsubsection{SRAM}
The purpose of the SRAM test is to see how reliable the SRAM memory is. In figure \ref{SRAM_test1} you can see current vs dose for the internal Large SRAM and the internal micro SRAM. 
\begin{figure}[!htbp]
\centering
  \begin{subfigure}{.49\textwidth}
  \centering
