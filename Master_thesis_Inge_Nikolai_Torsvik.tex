...
It is therefore of highest importance to test everything that is planned to be used in the \ac{ALICE} for radiation.
...
A \acf{SEL} is a phenomenon where a low resistance path between power and ground is formed, causing large current to flow. Normally latches will cause burned interconnections, which means reduced performance or destruction of a chip. \ac{SEL} can be discovered by measuring current of a component, and can be seen as large jumps in current. The only way to counter a latchup is by turning power off, and that is before the high current will burn interconnections and permanently set the component out of function.
...
For each of the different components, except the \ac{SF2}, a simple \acf{PCB} was used as a platform to be able to send inputs and measure output data.
One to two USB-DAQ (Data Acquisition) devices from National Instruments, were used to connect to the PCBs. These gave us the possibility to control the inputs of each component and measure the outputs.
...
Two \acp{PCB} were made for each of the components that were going to be tested. This was to have more test data on each component, and as a precaution if a problem should occur with one of these.
...
On the RCU2, these boards are supposed to be used to make a stable signal out of a not so stable signal coming from an optical receiver. The signal which is sent into the receiver is a Manchester-coded signal consisting of clock and data. The clock signal is going to be used as readout clock for all of the RCU2s, so that the readout of all of the RCU2s are synchronized.
...
In figure \ref{PCB_board1}, \ref{PCB_board2} and \ref{PCB_board3} you find pictures of all of the different PCB boards, which have been made through this work.
...
To test the SF2 SRAM memory a test code is written in the SF2 FPGA. In short the code writes a known pattern to all addresses and constantly checks if the values changes. If they do a counter will be incremented for every SEU detected.
...
Clock Conditioning Circuit which consist of a PLL, multiplexers and divider circuit, is used to configure clock signals on the \ac{SF2}.
The Clock Conditioning Circuit can be accessed and configured through a register, so that the frequency can be changed without needing to reprogram the SF2.
...
To test a PLL on the SF2 the PLL's lock signal was monitored. The lock signal goes high if the output clock signal is in sync with the input, and since the input signnal is assumed to be stable since it comes from a crystal, we will therefore have a indication if the PLL is working by checking the lock signal. A lock signal was checked by sending the lock signal to an I/O pin and over to the monitoring board, where the lock signal was constantly checked for a falling edge.

\subsection{Configuration and Monitoring}
The SF2 M2S050-FG896 is a chip with 896 BGA pads distributed over a chip of 2.5 cm x 2.5 cm. This means that making a simple PCB is close to impossible. Therefore, to test this component, SF2 starter-kits were used. The starter-kit has serial communication to USB, which makes it possible to easily communicate with the SF2 chip from a computer. A numerous of GPIO pins are also available on the kit, which makes it possible to send data in and out from the SF2. 
...
The test setup for the SF2, consists of USB to serial communicating from a computer to the SF2, a current measurement board, connected with wires to measure current, and another starter-kit (monitoring board) was connected to GPIO pins to be able to control and check the Logical element design.
...
If the current on one of the supply voltages goes over a certain threshold, the bit on the microcontroller is set to go low, resulting in turning the 3.3 V regulator on the SF2-starter kit off. The 3.3 V is used to power all other regulators on the kit, therefore by turning this regulator off, the entire starter-kit turns off.
...
The focus of the visit to TSL was the SF2 SoC FPGA, but we also tested an optical receiver and an optical transceiver, which respectively are being used for TTC (timing, trigger and control) and for Detctor Data Link2 (DDL2).
We brought with us two RCU2s and a total of 7 starter-kits, where 3 of these were with a smaller package of SF2 (M2S050-FG484), and the rest were engineering samples versions of M2S050-FG896, which is the one used on RCU2.
...
In \autoref{INA210_both} we find graphs of current and voltage as function of dose for the two test boards for INA210. A different reaction to radiation can be seen for the two boards. On the first board the output voltage increase and on the second board the output voltage decreased. This is quite strange, since the two boards should be exactly alike. Maybe the diode located on the input to the component got effected by the radiation on one of the two test boards, increasing or decreasing the measured current. It may also just be process variation. The current characteristic as function of dose are quite alike, the only difference is that on the second test board the current starts to increase a little earlier. 
...
The SRAM test gave us approximately the expected numbers of SEUs, however we had some problems with the microcontroller subsystem during the test. The calculated cross section for large SRAM for run 1 and 2 are respectively are $3.68\pm0.64\cdot10^{-14}\frac{cm^2}{bit}$ and $3.37\pm0.59\cdot10^{-14}\frac{cm^2}{bit}$, and cross section for micro SRAM for run 1 and 2 are respectively $1.03\pm0.18\cdot10^{-14}\frac{cm^2}{bit}$ and $1.51\pm0.27\cdot10^{-14}\frac{cm^2}{bit}$. The calculated values for both large SRAM and micro SRAM from run1 and run2 are very close to each other, which supports the result. Compared to other SRAM chips that have been tested \cite{arild_master} \cite{roed_doctor}, the calculated cross section is in the same magnitude.
...
If another test should be done on the SF2 SRAM memory, this could been an improvement to the test.
...
To know how often we can expect to encounter errors for the parts of the SF2 which gave been tested, when the RCU2s will be mounted in the TPC, an new variable will be introduced called Mean time between Failure (MTBF). Mean time between failure tells us the expected time between two errors, or from start to the first error, and the equation is as can be seen in \autoref{MTBF}.
...
From the results presented in \autoref{cross_section_results}, it can be seen how often we can expect an error for the different parts of the SF2 which has been tested, assuming we uses all registers, all of the SRAMs, and only on PLL in the RCU2 design. Memory cells like SRAM and registers are known to be week for radiation, see \autoref{SEU_Section}, so the result wasn't any surprise. All of the SRAM memory and registers, will probably not be used, which means that the calculated MTBF will in reality be much higher.

A PLL lock-loss will happen every third hour according to the acquired data, and that is with am assumption that only 1 PLL is used per RCU2. The effect of a PLL lock-loss, is still partially unknown, it may only result in one lost clock period, which probably wont harm to much, or it may be fall down for seconds, which means that useful data can be lost. More test should be performed on this aspect when we have a more complete firmware running on the RCU2, to see if the firmware tolerates to lose clock for a short period. 
...
In this section the results from the irradiation tests at TSL will be presented. The focus has been to test the SF2 SoC FPGA, but other tests will briefly be presented and discussed.
...
Normally when we see a decrease in voltage, we see an increase in current, or opposite, but in this case they both drop. This suggest that the voltage drops first, causing processes in the SF2 to shut down, which results in decrease in current. Whats causing this effect is unknown, but a theory could be that the DC-DC regulator, which controls the 1.2 V power source had some reaction to irradiation. The regulator is located right next to the SF2 chip, and it is therefore very exposed to radiation. Normally regulators have a gradually increase or decrease in the output voltage, as can be seen on both TPS51200 and MIC69302WU, but since this is a DC-DC regulator with programmable output voltage, we may have other effects, like the one that we detected.
...
For the test we exposed both the SF2 and the optical transceiver on the RCU2. 
For both of the test we detected few errors, less than 10 errors with a fluence of $3.18\cdot10^{11}\frac{n}{cm^2}$ when exposing the SF2 chip, and less than 5 errors when exposing the optical transceiver with a fluence of $3.6\cdot10^{11}\frac{n}{cm^2}$. This gives cross section of $3.14\cdot10^{-11} cm^2$ and $1.39\cdot10^{-11} cm^2$ respectively for when exposing the SF2 and the optical transceiver. 
...
For the TTC test, we exposed both the SF2 and the optical receiver.
When we exposed the SF2, we received PLL lock loss with about the same frequency as for the test with the Starter-kits, and the calculated cross section was $2.3\cdot10^{-10}\frac{\centi\meter^2}{PLL}$. Almost all the PLL errors was defined as PLL lock-loss due to bad PLLs. But when we started to expose the optical receiver, we detected PLL lock-loss on both of the PLLs all the time, we got a cross section of $6.9\cdot10^{-9}\centi\meter^2$.
...
The SRAM test on the two starter-kits was successfully performed, we had a lot of downtime due to errors, but the calculated cross section for both of our test were quite close, which supports the results. The average cross section for large SRAM is $3.03 \pm 0.45 \cdot10^{-14}\centi\meter^2$, and for micro SRAM is $1.26 \pm 0.19 \cdot10^{-14}\centi\meter^2$. This can be compared with the results from OCL, where a cross sections of $3.53 \pm 0.47\cdot10^{-14}\centi\meter^2$ for the large SRAM, and $1.27 \pm 0.16\cdot10^-{14}\centi\meter^2$ for the micro SRAM were found. There is a noticeable difference in energy level from OCL to TSL, and we would therefore examine if we should expect a difference in the calculated cross sections.

SEU occur due to ionizing particles charge generation close to sensitive nodes. Proton has in itself not enough ionizing capability to directly cause a SEU. But protons can have nuclear reactions with silicon or other material in a component, which then can have enough ionizing capability to create a SEU. \cite{roed_master} shows that cross section for proton-silicon interaction has a peak value around 20 MeV. But a component consist of more than only silicon, which have other cross sections for interaction with proton We will also with higher energies have higher chance of having secondary radiations and the ionizing capability of the secondary radiation increases as well. There is therefore hard to say for which energy to expect highest cross section, since there are effects pointing in both direction. The results shows a little lower cross section for a 170 MeV beam compared to a 25 MeV beam, which means that we could expect a cross section in the same area.
...
For the test of the logic element, we wanted to see if SET could be a problem on the SF2. SET was supposed to be detected by getting a higher cross section for higher frequency on the shift-register design. However the results as presented inn \autoref{Shift_register_data} doesn't show a increase with higher frequency, and we are therefore not able to distinguish between SEU and SET. We got some SEUs though and It is assumed that this was caused by  we lacked sufficient data to be able to distinguish between SEU in the registers and SET. 
...
TTC PLL when SF2 irradiated & $2.3\cdot10^{-10}$ & $\sim1.86$ \\ \hline% 176 min
TTC PLL when optical receiver irradiated & $6.9\cdot10^{-9}$ & $\sim0.06$ \\ \hline% 4 min
Latchup SF2 irratiated 1.2 V & $8.5\cdot10^{-10}$ & $\sim0.5$ \\ \hline% 4 min
Latchup SF2 irratiated 1.1 V & $1.7\cdot10^{-10}$ & $\sim2.5$ \\ \hline% 4 min
...
% SEU and SET in registers 40 MHZ 0 inverters& $5.19\pm0.40\cdot10^{-14}$ & 0.15 \\ \hline%  17 minimum
...
The result from \autoref{cross_section_results2} tells us that every 4 minutes for run2 in the TPC we will have a PLL error due to radiation on the optical receiver. This is very often, and it could be a major problem if not countered or fixed in some way. The reason for this effect could also be discussed. What we know is that the lock signal should tell us if the input signal and output signal to the PLL is in sync. The cause of the a lock-lose, could be bad input signal, error in the PLL and bad sensing for the lock signal. It has previous been assumed that if the PLL lock-loss falls low, so those the clock signal. But this isn't necessarily the case. The PLL can still be sending out what it is able to produce.  are having problem getting lock on the clock signal, when the optical link is exposed to radiation. This means most likely that the optical receiver is producing some noise which the limiting amplifier are not able to suppress or the optical receiver is not able to convert the optical signal as it should under high radiation level.

Whats happening to the clock signal when the lock signal falls down could also be discussed. we have previous assumed that when the lock signal falls down, so those the clock signal, but this is necessary the case. The lock signal could also be only a indicator that the clock signal in and out are not in sync, this could mean that we are missing a clock pulse or several, or the input signal may only be out of phase. Effects like this means that measurement in the RCU2 will not happen when it is supposed to, which is a effect we would not want to have. So a new solution with the TTC should be found, which either fix the problem or reduce it drastically.

We also see that we could expect a latchup every half an hour in the TPC, if the normal 1.2 V core voltage is used. We can also see that time between each latchup increases by a factor 5 by reducing the core voltage to 1.1 V instead of 1.2 V. We don't know if 1.1 V is the absolute minimum we could go before functions in the SF2 stops working, and more test on this aspect should also be performed.

The MTBF results also shows that we would have a SEU in the large SRAM ever minutes, if all of the RCU2 memory is used. All of the memory
\FloatBarrier
