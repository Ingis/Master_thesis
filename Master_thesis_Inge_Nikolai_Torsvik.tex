...
\section{Conclusion}
The focus through this work has been to test components used in the design of the new RCU2 for radiation tolerance. A total of 11 components have been tested, where the focus has mainly been on checking the functionality and power consumption under radiation. A lot of radiation test has been executed, a total of 

It has been conducted research on the radiation level in the TPC detector, and it has been estimated that for the next run period (run2), a radiation dose of 1-2 kRad, with a flux of 3 $\kilo\hertz/\centi\meter^2$ can be expected for the electronics located in the detector. The acquired radiation results is compared up against these numbers, and used to determine if the tested components are cleared to be used on the RCU2.

Every component has been tested for TID effects and a part of the SmartFusion2 SoC FPGA has also been tested for \acl{SEE} like \acl{SEU}, \acl{SEL} and \acl{SET}.
