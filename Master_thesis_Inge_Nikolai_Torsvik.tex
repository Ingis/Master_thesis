...
\usepackage[paper=a4paper,pdftex,top=3cm,left=3cm,right=3cm, foot=1cm,bottom=3cm]{geometry}
\usepackage{picinpar}
...
\begin{center}        
  \vspace{5mm}        
  \huge
  \textbf{Radiation Testing of RCU2 Electronics} \\
  \vspace{50mm}
  \Large
...
For the RCU2 design we introduces new components, where some of these hasn't been tested for radiation tolerance. The work presented through this thesis comprises of irradiation tests of these components, and the preparation process before radiation tests. The tested components consist of power regulators, bus transceivers, limiting amplifier, multiplexer/demultiplexer, buffer, comparator, Current Shunt Monitor and the RCU2's main FPGA, the Microsemi SF2 M2S050-FG896. The tests performed consist mostly of test for accumulative effects, but also test for \ac{SEE} will be performed for some of the components.
The parts of the SF2 that were tested are SRAM, PLLs and Logic Element.

The radiation tests has been executed at \ac{OCL} in Oslo Norway, with a proton beam of 25 and 28 MeV and at \ac{TSL} in Uppsala Sweden, with a proton beam of 170 MeV.
...
The irradiation test was executed in four periods, three times at \ac{OCL} with a proton beam of 28 MeV and 25 MeV where all components have been tested, and one time at \ac{TSL} with a proton beam of 170 MeV, where only the SF2 SoC FPGA was tested. A \ac{SF2} starter-kit was used when designing test for this \ac{IC}, and test code was written in VHDL and C. To be able to detect SEL the current on the FPGA had to be measured. Therefore a current measurement board was made for that task.
...
The SRAM test gave us around the expected numbers of SEUs, but we had some problems with the MSS during the test. The calculated cross section for large SRAM and micro SRAM are respectively $4.18±0.73\cdot10^{-14}$ and $(9.62±1.32)\cdot10^-{15}$, which is around the same magnitude as other SRAMs which has been tested \cite{arild_master} and \cite{roed_doctor}, which supports the results.
