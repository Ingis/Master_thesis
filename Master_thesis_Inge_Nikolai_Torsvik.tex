...
This computer was used as a connection point to our test boards. We sat in a room called \emph{countingroom}, where we were able to switch the beam on and off as we desired. There were also a machine in the countingroom, which was connected to detectors in the blue hall telling us the fluence (number of protons per $\centi\meter²$ in total) of the beam. The fluence can be used to calculate the dose. The energy of the proton beam we got was 180 MeV.
...
The focus of the visit to TSL was the SF2 SoC FPGA, but we also tested a optical receiver and a detector data link used to send data out of the RCU2.
...
For the test discussed in section \ref{SF2_section}, we only used the SF2 starter-kits to test, since the test only involves internal parts of the SF2 SoC FPGA. Two of the starter-kits with the smaller package was tested M2S050-FG484, and one with engineering sample M2S050ES-T-FG896. When the RCU2 cards was irradiation, things like SERDES (DDL), trigger, clock and data recovery (CDR) and PLL was tested, which will not be discussed throughly in this work. Current was measured on both the SF2 starter-kits and RCU2 boards when they were irradiated to check for latchups.
...
Test of all the PCBs will be presented first, followed by a discussion on SF2 test. The result is presented after type, meaning that two PCBs containing the same \ac{IC} will be presented together. The purpose of these test has been to check for radiation tolerance. The focus has therefore been to check the output data/voltage and current compared with the received dose. Flux versus time for the different tests can be seen in the summery of this section \ref{flux_all1},\ref{flux_all2} and \ref{flux_all3}

An overview of the test can be found at the end of this section, with the total exposed time, the total irradiated dose and error status. Flux graph from all of the test can also be found.

\subsubsection{TPS51200}
This was the first component that was radiation tested, we started therefore out with a very low intensity, to be sure that everything worked as it should.
...
If we look at cross section per bit we get:

\begin{equation}
 CS_{LSRAM} = \frac{2.6*10-8}{} =
\end{equation}

\begin{equation}
 CS_{uSRAM} = \frac{7.0*10-10}{} =
\end{equation}

% Cross section for the whole period before the latchup was also calculated for reference. 
% 
% \begin{equation}
%  CS_{tot} = \frac{2453}{1.11e-{11}}=0,000000022 = 2.2*10-8
% \end{equation}
\FloatBarrier
...
On run two, we ran 4 shift register chains with shift register length of 2000, with 4 inverters in between each register and 4 windowed shift register. The frequency used was 40 Mhz. In table \ref{SR_table}, the results from run2 can be seen.

We detected 5 shift register errors in total over a dose of 8 kRad. That was something like we would expected, but we couldn't distinguish between upset or transient on the tracks, since we only got to run on one frequency. And then the whole point with this test didn't get tested. But we got at least a feeling on how often we would see an error on the shift register with the settings used.
...
Graphs of current versus dose can be found in appendix \ref{radiation_results}. We received much less latchups then expected. From Microsemi, we have heard roomers of a problem with micro latchup in the Cortex SRAM memory, which would increase current with a 50-100 mA, but wasn't destructive. We didn't detect any micro latchups during our test, but we got two rather big latchup, which doubled the current.

The first lock loss on the PLL occurred quite early in our test, already after a dose of 0,7 kRad. We got a total of 23 PLL lock loss.
