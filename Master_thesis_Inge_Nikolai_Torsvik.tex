...
In \autoref{PLL_error1} and \autoref{PLL_lock_loss} you can see the results from the PLL test for run1 and run2. The lock signal that was monitoring, was from a PLL that was driven by another PLL. Thus, assuming that if the first PLL loses its lock signal and thus clock, the second PLL will also lose its lock signal.

We have a large period in between a fluence of $1.47\cdot10^{10} \frac{n}{cm^2}$ and $5.45\cdot10^{10} \frac{n}{cm^2}$, where we didn't detect any PLL lock-loss, which is rather strange, and is assumed to be an error with the measurement. When calculating the cross section we will therefore calculate before and after this period, where the PLL lock-loss increases more linear with the fluence, see \autoref{PLL_lock1}.
