...
Graphs of current versus dose can be found in appendix \ref{radiation_results}. We received much less latchups then expected. From Microsemi, we had heard roomers of a problem with micro latchup in the Cortex SRAM memory, which would increase current with a 50-100 mA, but wasn't destructive. We didn't detect any micro latchups during our test, but we got two rather big latchup, which more than doubled the current, from around 270 $\milli\ampere$ to around 730 $\milli\ampere$ on the 1.2 V power source during run1 and in run2 we went from 400 $\milli\ampere$ to around 850 $\milli\ampere$ on the 1.2 V power source.  The nominal current for run2 was a little higher since the shift register design was also running.

In table \ref{PLL_error1} and \ref{PLL_error2} you can see the result from the PLL test. A noticeable things is that the first lock loss already occurred after a dose of 0,7 kRad. The The PLL that was monitored was driven by another PLL. Thus, assuming that if the first PLL looses its lock signal and thus clock, the second PLL will also loose its lock signal. We got a total of 23 PLL lock loss, giving us 11.5 loss per PLL. 
...
Two starter kit with the SF2 M2S050-FG484 were tested, and one starter-kit with the SF2 M2S050ES-FG896 (engineering sample). The SF2 M2S050-FG484 is the same Soc FPGA which we are going to use on the RCU2, only with smaller package. Therefore we preferred testing our code on these instead of the engineering sample version, which may have some early design errors. The things that were tested on the starter-kits with smaller package are SRAM, PLL, Logic element and latchup, as discussed in section \ref{SF2_section}. When testing the M2S050ES-FG896, the focus was only to see for how long we could program the device with increasing dose.

Some improvements has been done to the the test since radiation at OCL. We have added two more PLL lock signals signal to monitor, which didn't got measured in \ac{OCL}. And we found a more effective way to measure current. We used a ....... , which is a ADC with $I_2C$ communication protocol and an internal voltage reference. The SF2 have $I_2C$ communication protocol, which means that we could use the monitoring board to control and receive data from the ADC-chip. So we didn't need the current measurement board anymore, making the setup, a little easier. The monitoring board were also connected to the enable pin of the 3.3 V voltage regulator on the test board, making it possible to turn on and off the test board at will. 
