...
The SRAM test gave us around the expected numbers of SEUs. We should have tried to run the test without going through the MSS. If we had tried to send data directly out from the FPGA, we would get rid of all the probelem with the MSS which stoppet working all the time.
We had also quite many PLL errors which could be a rather big problem when the RCU2 will be mounted in the TPC. If the clock is not working, we would have a period where we don't have any data collection. And it is when there is much radiation, we want to collect data.
...
All in all the first radiation test of the SF2 did go quite good. We had some PLL errors, but we had a feeling that this would be a problem, so it wasn't any surprise. A way to counter this PLL problem, could maybe be to use PLLs in parallel, so that if one of them falls out, the other steps in.  
...
We could also here find the cross section. Since we have so many drop outs, because of latchup, current drops and power cycles, we have looked through the data log-file and got a estimated time with no data, and the flux in that period is known, which means that we can calculate the fluence where the measurment didn't work, when the board was off. This means that the actually cross section would probably be a little higher then the calculated values. As we did when calculating cross section after the result from OCL, we want to find a period which are as linear and error free as possible. Therefore we calculated the cross section in between 7.8 kRad to 14.3 kRad for test1 and 4.2 to to 18.5 on test2 for the large SRAM. Since we had so few SEUs on the micro SRAM for test1, we increased the statistics by checking from 0 to 14.8 kRad, even though this gave us more down time due to errors. For the micro SRAM on test2 we checked for the same period as for the large SRAM in between 4.2 kRad and 18.5 kRad. 


This gives us a cross section of:

\begin{equation}
 CS_{LSRAM} = \frac{SEUs}{FluenceDUT} = \frac{5466}{9.90\cdot10^{10}}= 4.51\cdot10^{-8}
 \label{Cross_section_sf2_tsl1}
\end{equation}

\begin{equation}
 CS_{LSRAM} = \frac{SEUs}{FluenceDUT} = \frac{4175}{2.26\cdot10^{11}}= 1.85\cdot10^{-8}
