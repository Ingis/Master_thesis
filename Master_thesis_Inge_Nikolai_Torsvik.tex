...
This chapter will first explain how the data collected after a radiation test was used to calculate flux and dose, followed by a presentation of the result.
...
From the calibration before beam test, we got the relation between scintillator counts and SEU on the Neutron dosimeter. And from earlier tests the cross section for a \ac{SEU} on the Neutron dosimeter board is known. These data is used to calculate our way to the received dose for each of the different \ac{IC}. The next section will explain the calculation process.

At \ac{TSL} there was a permanently beam monitoring setup, which means that we only had place the component that we wanted to test in the beam, and all data like flux and dose we got from the personnel working there. At \ac{OCL} however, we had to do monitor and calculate everything ourself. The next sections regarding dose calculation applies manly for testing at \ac{OCL}.

\section{Calculation of dose}
Two ways to calculate dose will be discussed in this section, that is manually using stopping power and \ac{LET}, see section \ref{radiation_phys}, and using simulation data from a program named FLUKA. Which of these gives us the most reliable result will be discussed at the end of this chapter.
...
The particle energy is normally expressed in electronvolt ($\electronvolt$) or Megaelectronvolt ($\mega\electronvolt$). One electronvolt is defined as the amount of kinetic energy gained by a single unbound electron when it accelerates through an electric potential of one Volt, thereof $\electronvolt$. Its value is electric charge, $e = 1.602 \times 10^{-19}$, multiplied with one Volt, which equals $1.602 \times 10^{-19}\joule$, or expressed for  $\mega\electronvolt$:

\begin{equation}
1 \mega\electronvolt = 1.602 \times 10^{-13}\joule
\label{MeV_to_joule}
\end{equation}

\subsection{Calculating dose using LET}
When a particle enters a material, energy is deposited in that material through ionization, and that is called \acf{LET}, \ref{LET_section}. To calculate the absorbed dose for a specific \ac{IC} after radiation, we have to first calculate the \ac{LET} to know the energy deposited in the \ac{IC}. But first of all we have to know the energy when it hits the \ac{DUT}, since it will lose energy from beam exit to \ac{DUT}-spot.

The first time at \ac{OCL} we got a 28 MeV proton beam, the two other times we got a 25 MeV proton beam, but we will also have some energy loss in air. By calculating the specific stopping power \ref{sstopping_power} in air, I used a program called \emph{Ionization loss}, we get respectively $17.516 \frac{\mega\electronvolt\centi\meter^2}{\gram}$ and $19.201 \frac{\mega\electronvolt\centi\meter^2}{\gram}$. By multiplying with the density of air which is $\rho_{air}=0.001275 \frac{\gram}{\centi\meter^3}$ we get $22.333 \frac{\kilo\electronvolt}{\centi\meter}$ and $24.481 \frac{\kilo\electronvolt}{\centi\meter}$. Distance between beam exit and \ac{DUT}-spot is $130 \centi\meter$, which gives us:

\begin{equation}
E_{proton1} = 28 - (130 \centi\meter \times 22.333 \frac{\kilo\electronvolt}{\centi\meter}) \approx 25 \mega\electronvolt
\label{Proton_energy1}
\end{equation}

\begin{equation}
E_{proton} = 25 - (130 \centi\meter \times 24.481 \frac{\kilo\electronvolt}{\centi\meter}) \approx 22 \mega\electronvolt
