...
The dose that we could expect at CERN for a 10 year period in the ALICE detector is estimated to be approximately 0.6 kRad from Pb-Pb collisions that will be run 1 month a year and a little higher for p-p collisions that will be run 10 months a year \cite{georgios} and \cite{roed}.
...
One of the biggest attractions at \ac{CERN} is the \acf{LHC}, that is a circular particle accelerator placed in a 27 km long tunnel around 100 meter beneath ground level.
This is the last accelerator in a chain of up to 7, where the particles gradually accelerate to higher and higher energies,
up to their maximum energy of 4 TeV and speed up to the speed of light.
There are four main detectors placed in the \ac{LHC} tunnel, where \ac{ALICE} is one of them.
When the particles has reached a high enough energy level, accelerated particles from opposite direction are made to collide at the center of each detector.

\subsection{TPC and Front End Electronics FEE}
The \ac{ALICE} detector is a heavy ion detector. It is designed to study the quark-qluon plasma\footnotemark[1] which will be generated when heavy ions colliding.
The \ac{ALICE} detector consist of several sub-detectors, where \acf{TPC} is the main detector. Its function are tracking particles, measure the charged particles
momentum and particle identi?cation. A drawing of the \ac{ALICE} detector can be seen in figure \ref{ALICE_layout}. 
